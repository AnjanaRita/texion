...
class CapsNet(nn.Module):
    def __init__(self, routing_iterations, n_classes=10):
        ...
        self.primaryCaps = PrimaryCapsLayer(
            256, 32, 8, kernel_size=9, stride=2)  # outputs 6*6
        self.primaryCaps = PrimaryCapsLayer(
            256, 32, 8, kernel_size=9, stride=2)  # outputs 6*6
        ...
        self.digitCaps = CapsLayer(
            self.num_primaryCaps, 8, n_classes, 16, routing_module)
...
class MarginLoss(nn.Module):
    ...
    def forward(self, lengths, targets, size_average=True):
        ...
        losses = targets.float() * F.relu(self.m_pos - lengths).pow(2) + \
                 self.lambda_ * (1. - targets.float()) * \
                                 F.relu(lengths - self.m_neg).pow(2)
            self.lambda_ * (1. - targets.float()) * \
            F.relu(lengths - self.m_neg).pow(2)
...
if __name__ == '__main__':
    ...
    scheduler = lr_scheduler.ReduceLROnPlateau(
        optimizer, verbose=True, patience=15, min_lr=1e-6)
    scheduler = lr_scheduler.ReduceLROnPlateau(
        optimizer, verbose=True, patience=15, min_lr=1e-6)
        ...
        for data, target in test_loader:
            ...
            if args.with_reconstruction:
                ...
                reconstruction_loss=F.mse_loss(
                    output, data.view(-1, 784), size_average=False).data[0]
                reconstruction_loss=F.mse_loss(
                    output, data.view(-1, 784), size_average=False).data[0]
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        torch.save(model.state_dict(),
                   '{:03d}_model_dict_{}routing_reconstruction{}.pth'.format(epoch, args.routing_iterations,
                                                                             args.with_reconstruction))                                                                             args.with_reconstruction))
